  ...
  <article class="box" v-loading="loading">
    <el-form :model="form" class="p-5 col-12 col-lg-7 col-xl-9" label-position="labelPosition">
      ...
      <el-form-item label="选择头像：">
        <div class="headImg">
          <el-image v-for="(item, index) in imgSrc" :key="index" @click="imgClick(index)" :src="imgSrc[index]"
            :class="imgArr[index] ? 'activeImg' : ''" fit="cover">
            <template #error>
              <div class="headBox">
                <el-icon>
                  <Picture />
  ...
  export default {
    ...
    setup() {
      ...
      function submit() {
        ...
        axios
          .get(
            `https://lan-ze-user.vercel.app/api/user/userUpload?id=${id}&name=${form.name
            }&img=${sqlImg.value[lickImgIndex.value]}&intro=${form.intro}`
          )
          .then((response) => {
            axios.get(`https://lan-ze-user.vercel.app/api/user/getUpload?id=${id}`)
              .then((response) => {
